...
int USB1_OUT_Phase_GetDesc(struct usb_device_data *data, u32 data0, u32 data1, u32 len){
    u16 host_csr0 = 0;
    int ret = 0;
    u32 tmp[2];

    if (len == 8){
        USB1_WriteDataFIFO(data, 0, data0);
        USB1_WriteDataFIFO(data, 0, data1);
    }
    else if (len <= 4) {
        USB1_WriteDataFIFO(data, 0, data0);
    }
...
int USB1_WRITE_Transaction(struct usb_device_data *data, const u8* pkt, u16 addr, u8* string){
    ...
    if (ret == 0){
        if (data->InsReq.wLength) {
            ret = USB1_OUT_Phase_GetDesc(data, 0x00000008, 0x802500, 8);
        }
    }   
...
int USB1_GetDesc_Transfer(struct usb_device_data *data){
    ...
    if (ret == 0){
        ret = USB1_WRITE_Transaction(data, SetAddr_pkt, 0x0, "SetAddr_pkt");
    }
    ...
    if (ret == 0){
        ret = USB1_WRITE_Transaction(data, SetConf_pkt, 0x1, "SetConf_pkt");
    }
    ...
    if (ret == 0){
        ret = USB1_WRITE_Transaction(data, SetCoding_pkt, 0x1, "SetCoding_pkt");
    }

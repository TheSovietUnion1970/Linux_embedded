...
void USB1_Create_Cluster_Dir_LFN(u8* dir_name, u32 dir_name_len, LFN_Root_Dir_Entry* entry, u16* entry_num){
    ...
    *entry_num = (dir_name_len + 1)/13;
    ...
    //printk("dir_name_len = %d, entry_num = %d\n", dir_name_len, *entry_num);
...
void USB1_Create_Cluster_Dir(u8* cluster_data, u8* dir_name, u32 dir_name_len, u32 next_cluster_num, u16* bytes_occupied){
    ...
    if (dir_existed == true){
        ...
        memcpy(Glob_free_entry - Ins_entry_len, (u8*)&Ins_entry[0], 32*(Ins_entry_len + 1));
...
void USB1_Read_FAT(struct usb_device_data *data){
    ...
    (void)USB1_Read_SECTOR_DATA(data, bpb_instance.Starting_LBA + bpb_instance.Reserved_Sectors, 1, Glob_Sector_data, &Sector_data_len, "FAT content", 0, PRINT_DATA);
}

...
u8 Dir_name[20][100]; // up to 10 directories
u8 Dir_cluster[20];
u8 Dir_index = 0;
                    ...
                    if (*tmp_Root_Dir_Entry&END_MARKER){
                        ...
                        if (tmp_file_name[tmp_file_name_len - 3] == 't') { // 't'xt
                            ...
                            if (tmp_file_name[0] != '.'){
                                ...
                                USB1_Get_String(tmp_file_name, Dir_name[Dir_index], tmp_file_name_len);
                                Dir_cluster[Dir_index] = (valid_Dir_Entry[i]->High_first_cluster << 16) | (valid_Dir_Entry[i]->Low_first_cluster);
                                Dir_index++;
                            }
#endif
                            //printk("***********\n");
...
#if (!HIDDEN_FOLDERS)
                        /* DIR SFN only when dir entry is located at the first index of cluster 
                           DIR SFN + LFN when dir entry is not located at the first index of cluster */
...
int USB1_f_Read(struct usb_device_data *data, u8* path){
    u8 target_dir[100];
    u16 len = 0;
    // check valid path?
    len = USB1_Parse_Target(path, target_dir);
    printk("target_dir = '%s', len = %d\n", target_dir, len);

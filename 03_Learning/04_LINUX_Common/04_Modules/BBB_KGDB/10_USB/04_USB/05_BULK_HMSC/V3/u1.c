...
void USB1_Print_DeviceDescriptor2(struct usb_device_data *data){
    ...
    printk("# wTotalLength = 0x%x\n", data->InsDeviceDescriptor.wTotalLength);
...
int USB1_OUT_Phase_Bulk(struct usb_device_data *data, u8 epnum, u8 addr, const u8* dataX, u32 len){
    u16 host_csr0 = 0, txcsr = 0, usbcore_pwr = 0;
    int ret = 0, i = 0;
...
int USB1_IN_Phase_Bulk(struct usb_device_data *data, u8 epnum, u8 addr, u8* dataX, u16* len){
	...
	printk("Index = %x\n", ioread8(data->base_usb1core + MUSB_INDEX));
	printk("FIFOSZ = %x\n", ioread16(data->base_usb1core + MUSB_TXFIFOSZ));
	printk("FIFOADD = %x\n", ioread32(data->base_usb1core + MUSB_TXFIFOADD));
    printk("rxcsr = %x\n", ioread32(data->base_usb1core + 0x10 + MUSB_RXCSR));
    printk("MUSB_FIFOSIZE = %x\n", ioread32(data->base_usb1core + 0x10 + MUSB_FIFOSIZE));
    printk("MUSB_POWER = %x\n", ioread32(data->base_usb1core + MUSB_POWER));
	printk("MUSB_RXTYPE = %x, MUSB_RXINTERVAL = %x, MUSB_RXMAXP = %x, MUSB_RXCSR = %x\n", ioread8(data->base_usb1core + 0x10 + MUSB_RXTYPE),
																		 ioread8(data->base_usb1core + 0x10 + MUSB_RXINTERVAL),
																		 ioread16(data->base_usb1core + 0x10 + MUSB_RXMAXP),
                                                                         ioread16(data->base_usb1core + 0x10 + MUSB_RXCSR));
	printk("MUSB_TXTYPE = %x, MUSB_TXINTERVAL = %x, MUSB_TXMAXP = %x, MUSB_TXCSR = %x\n", ioread8(data->base_usb1core + 0x10 + MUSB_TXTYPE),
																		 ioread8(data->base_usb1core + 0x10 + MUSB_TXINTERVAL),
																		 ioread16(data->base_usb1core + 0x10 + MUSB_TXMAXP),
                                                                         ioread16(data->base_usb1core + 0x10 + MUSB_TXCSR));
...
int USB1_Send_INQUIRY(struct usb_device_data *data){
    int ret, i = 0;
    u8 inquiry_data[36] = {0};  // SCSI response buffer
    u16 InDataLen;
    u8* ptr;

    // Prepare CBW
    data->usb1_cbw.dCBWSignature = 0x43425355;
    data->usb1_cbw.dCBWTag = 0x1; // Increment per command
...
// data->usb1_cbw.dCBWSignature = 0x43425355;
// data->usb1_cbw.dCBWTag = 0x1; // Increment per command
// data->usb1_cbw.dCBWDataTransferLength = 0x12; // Data-IN length (18 bytes)
// data->usb1_cbw.bmCBWFlags = 0x80; // IN direction
// data->usb1_cbw.bCBWLUN = 0;
// data->usb1_cbw.bCBWCBLength = 6;
// data->usb1_cbw.CBWCB[0] = 0x03; // REQUEST SENSE opcode
// data->usb1_cbw.CBWCB[1] = 0x00; // Reserved
// data->usb1_cbw.CBWCB[2] = 0x00; // Reserved
// data->usb1_cbw.CBWCB[3] = 0x00; // Reserved
// data->usb1_cbw.CBWCB[4] = 0x12; // Alloc length=18
// data->usb1_cbw.CBWCB[5] = 0x00; // Control=0

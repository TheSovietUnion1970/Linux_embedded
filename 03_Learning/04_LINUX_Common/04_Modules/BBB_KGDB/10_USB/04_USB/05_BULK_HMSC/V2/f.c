...
/* Global buffer for reading content of file / directory */
u8 Cluster_data_ins[8][SECTOR_SIZE];

int USB1_Scan_Cluster(struct usb_device_data *data, u8* cluster_data, u8 cluster_num, bool print_data);
...
void USB1_Scan_ClusterData(struct usb_device_data *data, u8* cluster_data, bool print_data){
    ...
    // /* Local buffer for reading content of file / directory */
    // u8 Cluster_data_ins[8][SECTOR_SIZE];
    ...
    if (print_data){
        for (i = 0; i < valid_Dir_Entry_index; i++){
            if (!valid_Dir_Entry[i]) {
                ...
                // Dir type
                else if (valid_Dir_Entry[i]->File_attributes == DIR_TYPE){
                    if (ret == 0){
                        u8 next_cluster = 0;
...
int USB1_Read(struct usb_device_data *data){
    ...
    // /* Scan from Cluster 2 */
    // if (ret == 0){
    //     ret = USB1_Scan_Cluster(data, (u8*)Sector_data_Root_Dir_Entry, 0x02, 0);
    // }

...
u8 tmp_File[MAX_FULL_PATH_LEN];
u16 tmp_File_len = 0;
u8 tmp_Dir[MAX_FULL_PATH_LEN];
u16 tmp_Dir_len;
/* Global buffer for padding */
u8 padding_buffer[MAX_PATH_LEN];
u8 tmp_file_name[MAX_PATH_LEN];
...
void USB1_Scan_ClusterData(struct usb_device_data *data, u8* cluster_data, u8 cluster_num, bool print_dir, bool all_dir){
    ...
    Root_Dir_Entry* valid_Dir_Entry[MAX_DIRS];
        ...
        if (!valid_Dir_Entry[i]) {
            ...
            if (valid_Dir_Entry[i]->File_attributes == FILE_TYPE) { 
                ...
                if ((ret == 0)){
                    ...
                    if (print_dir) USB1_Print_String((u8*)Cluster_data, valid_Dir_Entry[i]->File_size, padding_buffer);
                }       
            }
#endif
            ...
            if ((valid_Dir_Entry[i]->File_attributes == LFN_TYPE) && (((valid_Dir_Entry[i]->id)&SEQ_NUM) == 0x01)) {
                ...
                if (valid_Dir_Entry[i + 1]->File_attributes == FILE_TYPE) {
                    ...
                    if (print_dir) USB1_Print_String(tmp_file_name, tmp_file_name_len, padding_buffer);
                }
                else {
#if (HIDDEN_FOLDERS)
            ...
            // valid_Dir_Entry[i]->id != '.' to prevent cluster N 0x2E at index 0 and 1
                ...
                if (ret == 0){
                    ...
                    if (ptr8[11] != 0x0F){ // print SFN only when LFN is non-existent
                        if (print_dir) USB1_Print_String((u8*)valid_Dir_Entry[i], 11, padding_buffer);
...
int USB1_f_Read_File(struct usb_device_data *data, u8* path_file){
    u8 File_name[MAX_DIRS];
    u8 Dir_name[MAX_DIRS];
...
bool USB1_Check_Existing(u8* cluster_data, u8* name, u32 name_len, u8 type){
    ...
    while(entry->id){
        if ((entry->File_attributes == type) && (entry->id != 0xE5)){
            USB1_Read_NameFile(entry, tmp_File, &tmp_File_len);

            ret = USB1_Compare_String(tmp_File, name, name_len);
            if (ret == 0){
                return true;
            }
        }
...
bool USB1_Check_Deleted(u8* cluster_data, u8* name, u32 name_len, u8 type, Root_Dir_Entry** output_entry){
    ...
    while(entry->id){
        ...
        if ((entry->id == 0xE5) && (entry->File_attributes == type)){
            USB1_Read_NameFile((Root_Dir_Entry*)entry, tmp_File, &tmp_File_len);
...
int USB1_f_Make_Dir(struct usb_device_data *data, u8* path_dir){
    u8 Dir_name1[MAX_DIRS];
    u8 Dir_name2[MAX_DIRS];
        ...
        if (ret == 0){
            ...
            while (SFN_entry->id){
                if ((SFN_entry->id != 0xE5) && (SFN_entry->File_attributes == FILE_TYPE)){
                    USB1_Read_NameFile(SFN_entry, tmp_File, &tmp_File_len);

                    ret = USB1_Compare_String(tmp_File, File_name, tmp_File_len);
                    if (ret == 0){
                        file_existed = true;
                        Target_SFN_entry = SFN_entry;

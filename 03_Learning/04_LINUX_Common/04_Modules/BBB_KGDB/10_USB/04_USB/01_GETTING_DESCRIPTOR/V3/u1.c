...
/* Forth, getting configuration */
int USB1_SETUP_Phase_GetConf(struct usb_device_data *data, u16 addr){
    u16 host_csr0 = 0;
    int ret = 0;
    ...
    USB1_ClrToken(data);
    USB1_SetToken(data, GetConf_pkt);
    USB1_ApplyToken(data, 0); // Load the 8 bytes of the required Device request command into the Endpoint 0 FIFO
    ...
    printk("data->InsReq.wLength = 0x%x\n", data->InsReq.wLength);
    ...
    if (host_csr0 == MUSB_CSR0_H_ERROR) {
        dev_info(data->dev, "ERROR - SETUP - GetConf\n"); // send additional 2 times
        return -1;
    }
    if (host_csr0 == MUSB_CSR0_H_NAKTIMEOUT) {
        dev_info(data->dev, "NAK_TIMEOUT\n"); // .... consider later
    ...
    // set length for getting descriptor
    // data->InsReq.leftLength = data->InsReq.wLength; // 9 bytes
    // data->ConfigurationDescriptorPtr = (u8*)(&data->InsDeviceDescriptor);

    return 0;
}

int USB1_IN_Phase_GetConf(struct usb_device_data *data){
    u16 host_csr0 = 0;
    int ret = 0;
    u32 tmp[2];
...
int USB1_STATUS_Phase_GetConf(struct usb_device_data *data){
    u16 host_csr0 = 0;
...
/* Fifth, setting configuration */
int USB1_SETUP_Phase_SetConf(struct usb_device_data *data){
    u16 host_csr0 = 0;
    int ret = 0;
    ...
    USB1_ClrToken(data);
    USB1_SetToken(data, SetConf_pkt);
    USB1_ApplyToken(data, 0); // Load the 8 bytes of the required Device request command into the Endpoint 0 FIFO

    iowrite16(0x1, data->base_usb1core + MUSB_TXFUNCADDR);
    ...
    if (host_csr0 == MUSB_CSR0_H_ERROR) {
        dev_info(data->dev, "ERROR in SetConf - SETUP\n"); // send additional 2 times
        return -1;
    }
    if (host_csr0 == MUSB_CSR0_H_NAKTIMEOUT) {
        dev_info(data->dev, "NAK_TIMEOUT\n"); // .... consider later
...
int USB1_STATUS_Phase_SetConf(struct usb_device_data *data){
    u16 host_csr0 = 0;
    ...
    if (host_csr0 == MUSB_CSR0_H_ERROR) {
        dev_info(data->dev, "ERROR in SetConf\n"); // send additional 2 times
        return -1;
    }
    if (host_csr0 == MUSB_CSR0_H_NAKTIMEOUT) {
        dev_info(data->dev, "NAK_TIMEOUT\n"); // .... consider later
...
int USB1_WRITE_Transaction(struct usb_device_data *data, const u8* pkt, u16 addr, u8* string){
...
int USB1_GetDesc_Transfer(struct usb_device_data *data){
    ...
    ret = USB1_READ_Transaction(data, (u8*)&GetDesc_pkt, 0x0, "GetDesc_pkt");

    /* Setting address 0x1 with default address 0 */
    if (ret == 0){
        ret = USB1_WRITE_Transaction(data, (u8*)&SetAddr_pkt, 0x0, "SetAddr_pkt");
    }

    /* Getting descriptor with new address 0x1 */
    if (ret == 0){
        ret = USB1_READ_Transaction(data, (u8*)&GetDesc_pkt, 0x1, "GetDesc_pkt");
    }

    /* Getting descriptor with new address 0x1 */
    if (ret == 0){
        ret = USB1_READ_Transaction(data, (u8*)&GetDesc_pkt2, 0x1, "GetDesc_pkt2");
    }

    /* Getting descriptor (Interface 0: Communications Class (CDC)) with new address 0x1 */
    if (ret == 0){
        ret = USB1_READ_Transaction(data, (u8*)&GetDescif0_pkt, 0x1, "GetDescif0_pkt");
    }

    // /* Getting configuration with new address 0x1 */
    // ret = USB1_SETUP_Phase_GetConf(data, 0x1);

    // if (ret == 0){
    //     do {
    //         ret = USB1_IN_Phase_GetConf(data);
    //     } while(data->InsReq.leftLength && (!ret));
    // }

    // if (ret == 0){
    //     ret = USB1_STATUS_Phase_GetConf(data);
    // }

    // /* Setting configuration with new address 0x1 */
    // if (ret == 0){
    //     ret = USB1_SETUP_Phase_SetConf(data);
    // }

    // if (ret == 0){
    //     ret = USB1_STATUS_Phase_SetConf(data);
    // }

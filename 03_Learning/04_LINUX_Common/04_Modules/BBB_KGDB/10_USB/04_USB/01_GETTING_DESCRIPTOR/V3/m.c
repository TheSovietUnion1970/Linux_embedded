...
static void re_request_irq_work(struct work_struct *work){
    ...
    // ret = USB1_OUT_Phase_Interrupt(data, 4, 0x1, OutData, sizeof(OutData));

    // // read data from arduino device
    // while(!data->isEnd) {
    //     if (ret == 0) ret = USB1_IN_Phase_Interrupt(data, 3, 0x1, InData, &InDataLen);
    //     if (ret < 0) break;
    //     if (data->isEnd) break;
    // };
    ...
    ret = USB1_IN_Phase_Interrupt(data, 1, 0x1, InData, &InDataLen);
    // print_arr(InData, InDataLen, "InData");

    printk("button = 0x%x, dx = %d, dy = %d, dz = %d\n", data->button, data->dx, data->dy, data->dz);

...
static int musb_ulpi_write(struct usb_phy *phy, u32 val, u32 reg)
{
	...
	printk("ULPIw - (u8)reg = 0x%x, (u8)val = 0x%x\n", (u8)reg, (u8)val);
		...
		const struct musb_fifo_cfg *cfg, u16 offset)
{
	...
	case FIFO_RXTX:
		...
		printk("epnum = %d, c_size = 0x%x, c_off = 0x%x\n", hw_ep->epnum, c_size, c_off);
...
static int musb_core_init(u16 musb_type, struct musb *musb)
{
	...
	printk("_V: %s: ConfigData=0x%02x (%s)\n", musb_driver_name, reg, aInfo);
	...
	for (i = 0; i < musb->nr_endpoints; i++) {
		...
		//printk("_V: musb->io.ep_offset(i, 0) = 0x%x", musb->io.ep_offset(i, 0));
...
musb_init_controller(struct device *dev, int nIrq, void __iomem *ctrl)
{
	...
	//printk("musb->io.ep_offset(0,0) = 0x%x!!\n", musb->io.ep_offset(0, 0));
	//printk("musb->io.fifo_offset(0) = 0x%x!!\n", musb->io.fifo_offset(0));
	/*
	...
	printk("_V: 3. musb->isr = %x, musb->nIrq = %x\n", musb->isr, musb->nIrq);
	/* attach to the IRQ */

...
start:
		...
		if (!hw_ep->tx_channel){
			//printk("irq = %d, tx = 0x%x, rx = 0x%x, usb = 0x%x\n", irq, musb->int_tx, musb->int_rx, musb->int_usb);
			printk("START ... ->\n");
			printk("mbase = 0x%x\n", mbase);
			printk("core -> 0x%x 0x%x 0x%x 0x%x\n", ioread32(mbase + 0x0), ioread32(mbase + 0x4), ioread32(mbase + 0x8), ioread32(mbase + 0xc));
			printk("ep0 csr -> 0x%x 0x%x 0x%x 0x%x\n", ioread32(mbase + 0x10), ioread32(mbase + 0x14), ioread32(mbase + 0x18), ioread32(mbase + 0x1c));
			printk("devctl + BABBLE -> 0x%x\n", ioread16(mbase + 0x60));
			printk("FIFOSZ -> 0x%x\n", ioread16(mbase + 0x62));
			printk("FIFOADDR -> 0x%x\n", ioread32(mbase + 0x64));
			// printk("ep0 fifo -> 0x%x 0x%x 0x%x 0x%x\n", ioread32(mregs + 0x20));			
			...
			u8 *buf, u32 offset, u32 len)
{
	...
	printk("_V: %s hw%d urb %p spd%d dev%d ep%d%s "
				"h_addr%02x h_port%02x bytes %d",
			is_out ? "-->" : "<--",
			epnum, urb, urb->dev->speed,
			qh->addr_reg, qh->epnum, is_out ? "out" : "in",
...
irqreturn_t musb_h_ep0_irq(struct musb *musb)
{
	...
	if (unlikely(!urb)) {
		...
		printk("ep0_irq: tx = 0x%x, usb = 0x%x, pwr = 0x%x\n", (ioread16(mbase + MUSB_INTRTX)), (ioread16(mbase + MUSB_INTRUSB)), (ioread16(mbase + MUSB_POWER)));

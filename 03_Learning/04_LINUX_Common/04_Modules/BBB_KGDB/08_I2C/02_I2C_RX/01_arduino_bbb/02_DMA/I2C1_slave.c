...
void Dma_read(struct i2c_device_data* data){
    void *kbuf;
    ...
    kbuf = dma_alloc_coherent(data->dev, data->byte, &dma_dst_addr, GFP_KERNEL | GFP_DMA);
    if (!kbuf) {
        dev_err(data->dev, "Failed to allocate DMA-coherent buffer\n");
        return;
    }
    memset(kbuf, 0x40, data->byte);

    reinit_completion(&data->rx_completion);
    rx_desc = dmaengine_prep_slave_single(data->rx_chan, dma_dst_addr, data->byte,
    ...
    //((char*)kbuf)[data->data_count++] = ioread32(data->base + I2C_DATA);
...
free_buf:
    dma_free_coherent(data->dev, data->byte, kbuf, dma_dst_addr);

    printk("Buffer received (%d):\n", data->byte);
    for (i = 0; i < data->byte; i++){
...
static int i2c1_slave_open(struct inode *inode, struct file *file)
{
...
static ssize_t i2c1_slave_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
...
static const struct file_operations i2c_device_fops = {
    .owner = THIS_MODULE,
    .open = i2c1_slave_open,
    .read = i2c1_slave_read,
};

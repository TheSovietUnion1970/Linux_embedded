...
void Dma_write(struct spi_device_data* data){
    ...
    //memcpy(kbuf, 0x40, count); // Copy data (e.g., 'D' or user input)
    memset(kbuf, 0x40, count);


    reinit_completion(&data->tx_completion);
    tx_desc = dmaengine_prep_slave_single(data->tx_chan, dma_addr, count,
...
static int spi0_configure_dma(struct spi_device_data *data)
{
    ...
    if (data->tx_chan) {
        ...
        tx_conf.dst_addr = (dma_addr_t)(data->base + MCSPI_TX0);

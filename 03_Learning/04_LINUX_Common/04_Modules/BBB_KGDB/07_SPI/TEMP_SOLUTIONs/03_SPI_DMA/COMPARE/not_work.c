...
static int spi_send_task(void *d)
{
    ...
    while (!kthread_should_stop()) {
        ...
        dma_addr = dma_map_single(data->dev, buf_test, count, DMA_TO_DEVICE);
        ...
        iowrite32(((char*)buf_test)[0], data->base + MCSPI_TX0); // write the number of bytes defined in TX_TRIGGER to trigger DMA request (here TX_TRIGGER is 1, so write 1 byte manually)
...
static int spi_device_probe(struct platform_device *pdev)
{
    ...
    chconf |= 1 << 27; // The FIFO buffer is used to transmit data.

    iowrite32(chconf, data->base + MCSPI_CHCONF0);

    iowrite32(0, data->base + MCSPI_XFERLEVEL); // interrupt at least 3 bytes

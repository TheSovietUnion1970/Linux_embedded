...
void Dma_read(struct spi_device_data* data){
    ...
    kbuf = dma_alloc_coherent(data->dev, DMA_SIZE, &dma_dst_addr, GFP_KERNEL | GFP_DMA);
    if (!kbuf) {
    ...
    ret = wait_for_completion_timeout(&data->rx_completion, msecs_to_jiffies(2000));
    if (ret == 0) {
...
free_buf:
    dma_free_coherent(data->dev, DMA_SIZE, kbuf, dma_dst_addr);
...
//printk("irq-> '%c'\n", ioread32(data->base + MCSPI_RX0));
    ...
    if ((ioread32(data->base + MCSPI_RX0) == '\0') || (ioread32(data->base + MCSPI_RX0) == '\n')){
        i = data->index;
        data->index = 0;

        printk("rx = '%s', i = %d, jump_num = %d\n", rx_data, i, data->jump_num);
...
static int spi_device_probe(struct platform_device *pdev)
{
    ...
    data->rx_chan = dma_request_chan(&pdev->dev, "rx111");

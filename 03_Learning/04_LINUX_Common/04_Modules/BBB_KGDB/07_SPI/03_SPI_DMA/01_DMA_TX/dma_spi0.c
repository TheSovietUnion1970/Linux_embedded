...
u8 glob_TX[LOOPs] = {0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49};
...
static void spi0_tx_dma_callback(void *data)
{
    ...
    // dev_info(spi0->dev, "TX DMA callback called\n");
    spi0->DMA_IRQ = 1;
}
...
void Dma_write(struct spi_device_data* data){
    ...
    kbuf = dma_alloc_coherent(data->dev, count, &dma_addr, GFP_KERNEL | GFP_DMA);
    if (!kbuf) {
        dev_err(data->dev, "Failed to allocate DMA-coherent buffer\n");
        return;
    }
    dev_info(data->dev, "Write: Preparing %zu bytes\n", count);
    memcpy(kbuf, glob_TX, count); // Copy data (e.g., 'D' or user input)
    //memset(kbuf, 0x40, count);
    ...
    iowrite32(((char*)kbuf)[0], data->base + MCSPI_TX0); // write the number of bytes defined in TX_TRIGGER to trigger DMA request (here TX_TRIGGER is 1, so write 1 byte manually)
...
free_buf:
    dma_free_coherent(data->dev, count, kbuf, dma_addr);
...
static int spi_device_probe(struct platform_device *pdev)
{
    ...
    data->tx_chan = dma_request_chan(&pdev->dev, "tx111");

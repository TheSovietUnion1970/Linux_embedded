...
#if (DMA_USED) /* DMA is used */
#define RX_TRIGGER 0 /* For DMA, it's must be fixed */
#else /* Non-DMA is used */
#define RX_TRIGGER 0 /* For non-DMA, we can adjust it */
#define FIFO_DRAINED 0
#endif
/*********************************************/
...
void Dma_read(struct spi_device_data* data){
    ...
    /* Do not need to read the first byte manually here due to it's already done by reading get_count */
...
static irqreturn_t irqHandler(int irq, void *d)
{
    ...
    if (irqsts & MCSPI_CHSTAT_RX0_FULL) {
        ...
        if (data->is_get_count == 0){
            ...
            dev_info(data->dev, "Please read /dev/spi1 to read data in non-DMA mode!\n");
#else
            if (ioread32(data->base + MCSPI_CHSTAT0) & MCSPI_CHSTAT_RXS) {
                data->rx_buffer[data->index++] = ioread32(data->base + MCSPI_RX0);
            }   

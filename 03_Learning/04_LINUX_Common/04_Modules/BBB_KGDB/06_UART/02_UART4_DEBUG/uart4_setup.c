...
static int bbb_uart_remove(struct platform_device *pdev)
{
    ...
    unregister_chrdev_region(data->devno, 1);
    release_mem_region(0x481a8000, 0x1000); // temporary to free the region to re-request the region later
...
static void re_request_irq_work(struct work_struct *work)
{
    ...
    dev_info(dev, "Workqueue: Attempting to re-request UART4 IRQ %d, pdev = %p\n", data->uart4_irq, pdev);

    bbb_uart_remove(pdev);
    dev_info(dev, "Workqueue: uart removed ... ===========================\n");

    msleep(2000);

    bbb_uart_probe(pdev, data);
    dev_info(dev, "Workqueue: uart installed ... ===========================\n");
...
static int irq_probe(struct platform_device *pdev)
{
    ...
    bbb_uart_probe(pdev, data);
    dev_info(dev, "UART4 IRQ driver initialized, pdev = %p\n", pdev);
    printk("=================\n");

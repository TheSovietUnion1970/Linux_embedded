...
static irqreturn_t irqHandler(int irq, void *d)
{
    ...
    // // no FIFO enabled -> iir[5:1] = 0x4 -> RHR interrupt
    if (2 == (iir>>1)&0x1F) uart->RX_buffer[uart->Tx_Rx_count++] = ioread32(uart->base + UART_RHR);
                              ...
                              size_t count, loff_t *ppos)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        //dev_info(uart->dev, "=> UART_THR = kbuf[i] ='%c'\n", kbuf[i]);  
    }
...
static void bbb_uart_init_hw(struct bbb_uart *uart)
{
    ...
    iowrite32(0x00, uart->base + UART_FCR);  /* Enable FIFO, clear TX/RX */

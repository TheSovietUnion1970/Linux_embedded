...
static int wait_register_DMA_update(struct can_device_data *data, u16 offset, u16 bit_offset, u8 bit_val, u16 delay_ms, u8* name_register){
    unsigned long timeout;
    timeout = jiffies + msecs_to_jiffies(delay_ms);

    while ((ioread32(data->base_edma + offset)&(1u << bit_offset)) != (bit_val << bit_offset))  // Wait register updated
    {
        if (time_after(jiffies, timeout)) {
            dev_err(data->dev, "Timeout DMA %s\n", name_register);
            return -ETIMEDOUT;
        }
        cpu_relax();
    } 
...
static int wait_register_DMA_val(struct can_device_data *data, u16 offset, u32 val, u16 delay_ms, u8* name_register){
    unsigned long timeout;
    timeout = jiffies + msecs_to_jiffies(delay_ms);

    while ((ioread32(data->base_edma + offset)) != val)  // Wait register updated
    {
        if (time_after(jiffies, timeout)) {
            dev_err(data->dev, "Timeout %s\n", name_register);
            return -ETIMEDOUT;
...
void Dma_read(struct can_device_data* data, u8 byte_num){
    ...
    //if1cmd = (ioread32(data->base + CAN_IF1CMD) >> 14)&0x1;
    printk("[1]DMA - DMAactive = 0x%x\n", if1cmd);
    ...
    ret = wait_for_completion_timeout(&data->if1_completion, msecs_to_jiffies(1000));
    if (ret == 0) {
    ...
    iowrite32(0x0, data->base_edma + param_addr + 0x1C);  /* CCNT */

    /* Clear any flags */
    iowrite32(1 << (ch - 32), data->base_edma + DMA_ECRH);  
    iowrite32(1 << (ch - 32), data->base_edma + DMA_EMCRH);  
    iowrite32(1 << (ch - 32), data->base_edma + DMA_SECRH); 

    /* Enable channel 40 */
    iowrite32(1 << (ch - 32), data->base_edma + DMA_EESRH);  /* EESR , HW-TRIGGER*/
    //iowrite32(1 << (ch - 32), data->base_edma + DMA_ESRH);  /* ESR , SW-TRIGGER*/

    // wait_register_DMA_val(data, DMA_IPR, ch, MS_DELAY, "DMA_IPR");
    //static int wait_register_DMA_update(struct can_device_data *data, u16 offset, u16 bit_offset, u8 bit_val, u16 delay_ms, u8* name_register)
    wait_register_DMA_update(data, DMA_IPR, ch - 32, 1, MS_DELAY, "DMA_IPR");
    //wait_register_update(data, DMA_IPR, ch - 32, 1, MS_DELAY, "DMA_IPR");
    // printk("DMA Wait ...\n");
    // msleep(100);

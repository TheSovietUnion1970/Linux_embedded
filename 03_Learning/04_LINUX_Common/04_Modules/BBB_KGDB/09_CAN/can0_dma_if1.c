...
void Dma_read(struct can_device_data* data, u8 byte_num){
    ...
    ret = wait_for_completion_timeout(&data->if1_completion, msecs_to_jiffies(MS_DELAY));
    if (ret == 0) {
    ...
    /* Clear any flags */
    iowrite32(1 << ch, data->base_edma + DMA_ECR);  
    iowrite32(1 << ch, data->base_edma + DMA_EMCR);  
    iowrite32(1 << ch, data->base_edma + DMA_SECR); 

    /* Enable channel 28 */
    iowrite32(1 << ch, data->base_edma + DMA_EESR);  /* EESR , HW-TRIGGER*/
    //iowrite32(1 << ch, data->base_edma + DMA_ESR);  /* ESR , SW-TRIGGER*/
...
static irqreturn_t irqHandler(int irq, void *d){
    ...
    if (Int0ID == 0x8000){ // Error and status interuupt
        if ((es&(1u << 8)) == 1u << 8){
            ...
            dlc = can_mctl&0xF;
            id = can_mctl&CAN_IFxARB_ID;
            printk("0x8000 [DMA] - raw1 = 0x%x, raw2 = 0x%x, dlc = 0x%x, id = 0x%x\n", raw_data1, raw_data2, dlc, id);
        ...
        dlc = can_mctl&0xF;
        id = (can_arb >> 18)&0x7FF;
        printk("msg obj [DMA] - cmd = 0x%x, raw1 = 0x%x, raw2 = 0x%x, dlc = 0x%x, id = 0x%x\n", cmd, raw_data1, raw_data2, dlc, id);
...
static int can0_probe(struct platform_device *pdev){
    ...
    data->base_edma = ioremap(EDMA_BASE, 0x1000);
    data->dev = &pdev->dev;
    data->increment = 0;
    data->count_many = 0;

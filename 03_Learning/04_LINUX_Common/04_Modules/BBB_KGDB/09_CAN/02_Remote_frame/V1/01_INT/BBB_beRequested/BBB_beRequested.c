...
void can0_receive_obj_Remote_Frames_Intclr(struct can_device_data *data){
    u32 if1cmd = 0, mctl = 0;
...
static irqreturn_t irqHandler(int irq, void *d){
    ...
    //Reading_received_msg(data, DMA_OBJ_MSG_NUM);
    ...
    printk("es = 0x%x, mctl = 0x%x, intpnd_x = 0x%x, id = 0x%x, pd12 = 0x%x\n", es, can_mctl, intpnd_x, id, ioread32(data->base + CAN_INTPND12));

    //can0_receive_obj_Remote_Frames_Intclr(data);
    // can0_receive_obj_Remote_Frames_read(data);

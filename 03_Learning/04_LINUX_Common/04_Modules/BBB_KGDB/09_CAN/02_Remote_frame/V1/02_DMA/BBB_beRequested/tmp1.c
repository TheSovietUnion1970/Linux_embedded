...
void can0_receive_obj_Remote_Frames_write(struct can_device_data *data, u8 msg_num){
    ...
    // config msg control, clear TxRqst for the next remote frame reception, also avoid consecutive transmit INT 
...
void can0_init(struct can_device_data *data) {
    ...
    // init transmit obj with remote frame configuration (no using DMA this time)
    can0_receive_obj_Remote_Frames_init(data, ID_CAN0, DMA_OBJ_MSG_NUM, 1);

    /* =============== DMA ================= */
#if(DMA_USED)
    DMA_probe(data); // init DMA side when CAN is in init mode
#endif
...
// #endif
    ...
    //can0_receive_obj_Remote_Frames_write_tmp(data, DMA_OBJ_MSG_NUM);

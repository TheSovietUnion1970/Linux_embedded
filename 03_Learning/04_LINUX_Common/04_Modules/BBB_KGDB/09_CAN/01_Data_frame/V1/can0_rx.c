...
void Reset_msg_obj(struct can_device_data *data, u8 msg_num, u8 msg_handler){
    ...
    //if1cmd &=~ (0xFF << 16); // [24]: WR_RD  = 1 (Write)
    if1cmd |= 1u << 23;
    if1cmd |= msg_num&(0xFF);
    iowrite32(if1cmd, data->base + CAN_IF1CMD);
...
static irqreturn_t irqHandler(int irq, void *d){
    ...
    if (Int0ID == 0x8000){ // Error and status interuupt
        if ((es&(1u << 8)) == 1u << 8){
            ...
            //can0_single_received_obj_Data_Frames(data, ID_received, 1, 1);
        }
        ...
        //can0_single_received_obj_Data_Frames(data, ID_received, Int0ID, 1);
        ...
        printk("msg obj - msgnum = 0x%x, raw1 = 0x%x, raw2 = 0x%x, dlc = 0x%x, id = 0x%x\n", Int0ID, raw_data1, raw_data2, dlc, id);
    }
    ...
    // //     }
    // //     if ((es&(1u << 4)) == 1u << 4){
    // //         //can0_single_received_obj_Data_Frames_msk(data, ID_received, 1, 1);
    //          Reading_received_msg(data, intpnd_x);

    //         //  can0_single_received_obj_Data_Frames(data, ID_received, 1, 1);
    //         //  //can0_read_received_message(data, 1, RX, &len);

    //         raw_data1 = ioread32(data->base + CAN_IF1DATA);
    //         raw_data2 = ioread32(data->base + CAN_IF2DATA);

    //         // printk("0x8000 - raw1 = 0x%x, raw2 = 0x%x\n", raw_data1, raw_data2); 
    // //     }
    // // }
    // // else { // msg object interrupt
    // //     can0_single_received_obj_Data_Frames(data, ID_received, Int0ID, 1);
    ...
    //      printk("msb obj - raw1 = 0x%x, raw2 = 0x%x\n", raw_data1, raw_data2);
    // // }
    ...
    // can_arb = ioread32(data->base + CAN_IF1ARB);
    // can_mctl = ioread32(data->base + CAN_IF1MCTL);
    // printk("cmd = 0x%x, arb = 0x%x, mctl = 0x%x\n",can_cmd, can_arb, can_mctl);
...
void can0_init(struct can_device_data *data) {
    ...
    for (i = 0; i < 64; i ++){
        Reset_msg_obj(data, i+1, 0); // reset msg obj including msgVal
...
static int can0_probe(struct platform_device *pdev)
{
    ...
    //can0_transmit_obj_Data_Frames_msk(data, ID_received, 1, 1);

...
static irqreturn_t irqHandler(int irq, void *d){
    ...
    printk("Int0ID = 0x%x, es = 0x%x, x = 0x%x\n", Int0ID, es, intpnd_x);

    /* Reset msgVal to prevent handled by msg handler */
    if (Int0ID != 0x8000) can0_transmit_obj_Data_Frames(data, ID_CAN0, Int0ID, 0); 
    //if (Int0ID != 0x8000) Reset_msg_obj(data, Int0ID, 0);
...
static void re_request_irq_work(struct work_struct *work)
{
    ...
    while (!atomic_read(&data->should_stop)){
        ...
        can0_transmit_obj_Data_Frames(data, ID_CAN0, 1, 1);

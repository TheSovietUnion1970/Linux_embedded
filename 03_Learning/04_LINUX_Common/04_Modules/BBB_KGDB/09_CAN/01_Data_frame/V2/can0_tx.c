...
static irqreturn_t irqHandler(int irq, void *d){
    ...
    if (Int0ID != 0x8000){ // after clearing status and error (below), data transfer interrupt is raised
        ...
        es = ioread32(data->base + CAN_ES);
        //can0_transmit_obj_Data_Frames(data, ID_CAN0, 5, 0); 
        /* Int0ID = 0x8000 */
    }

    //can0_transmit_obj_Data_Frames(data, ID_CAN0, 1, 0); 

    printk("Int0ID = 0x%x, es = 0x%x, can_mctl = 0x%x\n", Int0ID, es, can_mctl);
    //printk("0x%x 0x%x 0x%x 0x%x\n", msgval12, msgval34, msgval56, msgval78);
...
static void re_request_irq_work(struct work_struct *work)
{
    ...
    while (!atomic_read(&data->should_stop)){
        ...
        can0_transmit_obj_Data_Frames(data, ID_CAN0, 5, 1);

        txx = ioread32(data->base + CAN_TXRQ_X);
        tx12 = ioread32(data->base + CAN_TXRQ12);
        mux12 = ioread32(data->base + CAN_INTMUX12);
    
        printk("WORK => datx = 0x%x, dat12 = 0x%x, mux12 =0x%x\n",txx, tx12, mux12);

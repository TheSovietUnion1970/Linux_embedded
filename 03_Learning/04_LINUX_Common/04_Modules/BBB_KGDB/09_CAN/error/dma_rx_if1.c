...
void Dma_read_first(struct can_device_data* data, u8 byte_num){
    void *kbuf;
    ...
    if1_desc = dmaengine_prep_slave_single(data->if1_chan, dma_dst_addr, byte_num/4,
    ...
    //((char*)kbuf)[0] = ioread32(data->base + CAN_if1DATB);
...
static irqreturn_t irqHandler(int irq, void *d){
    ...
    printk("[1]IRQ - es = 0x%x, cmd = 0x%x\n", es, ioread32(data->base + CAN_IF1CMD));
#endif
...
void can0_init(struct can_device_data *data) {
    ...
    //iowrite32(bit_index(DMA_OBJ_MSG_NUM), data->base + CAN_if1UPD12); // choose automatic update
...
static int can0_probe(struct platform_device *pdev){
    ...
    data->if1_chan = dma_request_chan(&pdev->dev, "if1_dma");

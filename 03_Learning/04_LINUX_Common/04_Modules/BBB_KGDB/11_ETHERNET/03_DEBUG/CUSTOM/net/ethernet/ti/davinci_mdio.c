...
void Print_mdio_regs(struct davinci_mdio_data *data){
    u32 i = 0;
    int shareddata = 0;
    ...
    printk("--- [mdio regs] ---\n");
	for (i = 0; i < 6; i++){
		printk("# [%xh] = 0x%x\n", i*4, readl_relaxed((u8*)data->regs + i*4));
	}
...
static void davinci_mdio_init_clk(struct davinci_mdio_data *data)
{
	...
	data->clk_div = div;
	printk("[V] mdio_in = %d\n", mdio_in);
	printk("[V] data->pdata.bus_freq = %d\n", data->pdata.bus_freq);
	/*
...
static int davinci_mdio_probe(struct platform_device *pdev)
{
	...
	printk("[V] data->manual_mode = 0x%x\n", data->manual_mode);
	if (data->manual_mode) {

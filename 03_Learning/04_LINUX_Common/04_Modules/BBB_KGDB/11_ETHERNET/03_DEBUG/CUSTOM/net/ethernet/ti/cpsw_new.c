...
static void cpsw_set_promiscious(struct net_device *ndev, bool enable)
{
	...
	printk("[V1] cpsw_set_promiscious\n");
...
static void cpsw_ndo_set_rx_mode(struct net_device *ndev)
{
	...
	printk("[V1] cpsw_ndo_set_rx_mode\n");
...
// 	printk("[V1] cpsw_ndo_vlan_rx_add_vid\n");
...
void phy_link_change_V(struct phy_device *phydev, bool up)
{
...
void cpsw_slave_open(struct cpsw_slave *slave, struct cpsw_priv *priv)
{
	...
	struct device *d = &phy->mdio.dev;

	try_module_get(phy->mdio.bus->owner);
	try_module_get(d->driver->owner);

	phy->phy_link_change = phy_link_change_V;
	phy->attached_dev = priv->ndev;
	phy->state = PHY_READY;
	phy->adjust_link = cpsw_adjust_link;
...
// 	printk("[V1] cpsw_ndo_vlan_rx_kill_vid\n");
				       ...
				       size_t len)
{
	...
	printk("[V1] cpsw_ndo_get_phys_port_name\n");
...
static void cpsw_ndo_poll_controller(struct net_device *ndev)
{
	...
	printk("[V1] cpsw_ndo_poll_controller\n");
				   ...
				   struct netdev_phys_item_id *ppid)
{
	...
	printk("[V1] cpsw_get_port_parent_id\n");

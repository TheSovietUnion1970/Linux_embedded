...
void cpdma_intr_enable(struct ether_device_data* data){
    ...
    // channel 7 TX
    iowrite32((1u << 7), data->base_cpdma + TX_INT_SET);

    // channel 1 RX
    iowrite32((1u << 1), data->base_cpdma + RX_INT_SET);
}
...
void cpdma_intr_disable(struct ether_device_data* data){
    ...
    // channel 7 TX
    iowrite32((1u << 7), data->base_cpdma + TX_INT_CLEAR);

    // channel 1 RX
    iowrite32((1u << 1), data->base_cpdma + RX_INT_CLEAR);
}
...
void cpdma_submit(struct ether_device_data* data, u8* token, u8* buf, u16 len, u8 dir){
    dma_addr_t buffer;
    u32 mode;

    buffer = (dma_addr_t)buf;
    ...
    iowrite32((uintptr_t)token, &data->desc_dma->sw_token);
    iowrite32(buffer, &data->desc_dma->sw_buffer);
    iowrite32(len | CPDMA_DMA_EXT_MAP, &data->desc_dma->sw_len);

    // store desc into hdp
    iowrite32(data->desc_dma, data->base_txhdp + 4*7); // at channel 7
...
void test_send_packet(struct ether_device_data *data)
{
    ...
    cpdma_submit(data, test_packet, test_packet, 60, 1);  // dir=1 = to Port 1

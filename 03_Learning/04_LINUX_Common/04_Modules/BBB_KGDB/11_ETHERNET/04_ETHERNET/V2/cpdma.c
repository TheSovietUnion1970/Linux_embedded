...
int cpdma_ctlr_start(struct ether_device_data* data){
    ...
    printk("APPLY here 3, desc_dma_phys 3 = 0x%x\n", ioread32(data->base_rxhdp));
    //8 chan num
    for (i = 0; i < 8; i++){
...
/* DMA submit */
void cpdma_submit_rx(struct ether_device_data* data, dma_addr_t dma, u8* buf, u32 len, u8 dir, int ch, u8 idle){
    u32 mode;
...
int cpdma_rx_fill(struct ether_device_data* data){
    ...
    // set not idle to let desc written into rxhdp for the first time
    idle = false;

    // page is assigned to virtual data
    cpdma_submit_rx(data, dma, (u8*)page, CPSW_MAX_PACKET_SIZE, 0, chan_linear(data->rx_dma_channel), idle);
...
int rx_mq_poll(struct napi_struct *napi_rx, int budget){
    ...
    if (data->desc_dma_rx->sw_token && data->desc_dma_rx) {
        ...
        pa = page_address(data->desc_dma_rx->sw_token);

        payload = pa + CPSW_HEADROOM_NA;
    }
    ...
    cpdma_desc_free(data->desc_pool, &data->desc_dma_rx);
    ...
    cpdma_submit_rx(data, dma_buf, (u8*)new_page, CPSW_MAX_PACKET_SIZE, 0, chan_linear(data->rx_dma_channel), false);

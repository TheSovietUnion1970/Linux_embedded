...
void print_ipv4_header(const struct ipv4_header *ip) {
    ...
    printf("Source IP          : %u.%u.%u.%u\n",
           (ip->source_ip >> 24) & 0xFF, (ip->source_ip >> 16) & 0xFF,
           (ip->source_ip >> 8) & 0xFF,  ip->source_ip & 0xFF);
    printf("Destination IP     : %u.%u.%u.%u\n",
           (ip->dest_ip >> 24) & 0xFF, (ip->dest_ip >> 16) & 0xFF,
           (ip->dest_ip >> 8) & 0xFF,  ip->dest_ip & 0xFF);
}
...
void parse_and_print_packet(const uint8_t *packet, size_t len) {
    ...
    if (ip->protocol == 1) {  // ICMP
        if (len >= ip_hdr_len + sizeof(struct icmp_header)) {
            const struct icmp_header *icmp = (const struct icmp_header *)(packet + ip_hdr_len);
